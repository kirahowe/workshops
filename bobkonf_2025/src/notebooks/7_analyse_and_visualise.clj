...
(def with-temporal-components
  (-> dataset
      (tc/map-columns :year :date (comp jt/value jt/year))
      (tc/map-columns :month :date (comp jt/value jt/month))
...
(defn calculate-monthly-average [ds]
  (-> ds
      ...
      (tc/map-columns :date [:year :month] (fn [year month]
