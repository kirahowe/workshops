...
;; The first step of a data analysis is always to get a hold of the data. We'll gloss over a lot of these details in the interest of time, but for your information the details of how the data we're using here was collected are described below:
...
;; Now we have our raw data downloaded. For now we'll go ahead with the next steps, ands see how to make this more robust at the end. The next step is parsing the Excel file and transforming it into a format that's easier to work with for our analysis. We'll cover that in the next section.
